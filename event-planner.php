    ...
    <section id="portfolio-details" class="portfolio-details">
      <div class="container" data-aos="fade-up">
        ...
        <?php 
            if($event_data['state'] == '') { 
        ?>
            <h2 class="mt-5">This event is waiting for admin to determine it's Transmission cost!</h2>
        <?php } else if($event_data['state'] == 'pending') { ?>

        <h3>Accept Admin Transmission Cost for This event</h3>
        <p>(Admin Event Broadcast Transmission Cost: <?php echo $event_data['trans_cost'] ?> L.E)</p>
        ...
        <form method="post" class="m-auto">
            ...
            <div class="text-center">
                <input type="submit" class="btn btn-primary m-auto" value="Accept The Offer">
            </div>
            <?php
                if($_SERVER['REQUEST_METHOD'] == "POST") {
                    ...
                    if($get_money && mysqli_num_rows($get_money) > 0) {
                        ...
                        $deposit = $event_data['trans_cost'] * 0.1;
                        ...
                        if($add_money) {
                            $query = "update broadcasts set state = 'accepted' where id = '$event_id'";
                            $result = mysqli_query($con, $query);

                            if($result) {
                                echo "Successfully accepted the offer for this event!";
                            }
                        } else {
